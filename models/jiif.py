...
class JIIF(nn.Module):
    ...
    def __init__(self, args, feat_dim=128, guide_dim=128, mlp_dim=[1024, 512, 256, 128]):
        ...
        self.image_encoder = make_edsr_baseline(
            n_feats=self.guide_dim, n_colors=3)
        self.depth_encoder = make_edsr_baseline(
            n_feats=self.feat_dim, n_colors=1)
    ...
    def query(self, feat, coord, hr_guide, lr_guide, image):
        ...
        # LR centers' coords
        feat_coord = make_coord((h, w), flatten=False).to(
            feat.device).permute(2, 0, 1).unsqueeze(0).expand(b, 2, h, w)

        q_guide_hr = F.grid_sample(hr_guide, coord.flip(-1).unsqueeze(
            1), mode='nearest', align_corners=False)[:, :, 0, :].permute(0, 2, 1)  # [B, N, C]
        ...
        for vx in [-1, 1]:
            for vy in [-1, 1]:
                ...
                q_feat = F.grid_sample(feat, coord_.flip(-1).unsqueeze(
                    1), mode='nearest', align_corners=False)[:, :, 0, :].permute(0, 2, 1)  # [B, N, c]
                q_coord = F.grid_sample(feat_coord, coord_.flip(-1).unsqueeze(
                    1), mode='nearest', align_corners=False)[:, :, 0, :].permute(0, 2, 1)  # [B, N, 2]

                rel_coord = coord - q_coord
                rel_coord[:, :, 0] *= h
                rel_coord[:, :, 1] *= w

                q_guide_lr = F.grid_sample(lr_guide, coord_.flip(-1).unsqueeze(
                    1), mode='nearest', align_corners=False)[:, :, 0, :].permute(0, 2, 1)  # [B, N, C]
                q_guide = torch.cat(
                    [q_guide_hr, q_guide_hr - q_guide_lr], dim=-1)

                inp = torch.cat([q_feat, q_guide, rel_coord], dim=-1)

                pred = self.imnet(inp.view(B * N, -1)
                                  ).view(B, N, -1)  # [B, N, 2]
                preds.append(pred)
    ...
    def forward(self, data):
        image, depth, coord, res, lr_image = data['image'], data[
            'lr'], data['hr_coord'], data['lr_pixel'], data['lr_image']
        ...
        if self.training or not self.args.batched_eval:
            res = res + self.query(feat, coord, hr_guide,
                                   lr_guide, data['hr_depth'].repeat(1, 3, 1, 1))
        ...
        # batched evaluation to avoid OOM
            ...
            for start in range(0, N, n):
                ...
                ans = self.query(feat, coord[:, start:end], hr_guide, lr_guide, data['hr_depth'].repeat(
                    1, 3, 1, 1))  # [B, N, 1]
                tmp.append(ans)
...
if __name__ == '__main__':
    args = get_args()
    model = JIIF(args)
    replace_module(model, last_fp=False)
    print(model)
