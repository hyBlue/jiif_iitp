...
# sampling based
@torch.jit.script
def noise_quant(data: torch.Tensor,
                bit: torch.Tensor,
                alpha: torch.Tensor,
                is_training: bool,
                noise: bool,
                ...
                is_discretize: bool = True) -> torch.Tensor:
    ...
    if is_training and noise:
        if sym:
            ...
            with torch.no_grad():
                ...
                if hist.sum() != 0:
                    noise_t = torch.multinomial(
                        hist, data.numel(), True) + torch.rand_like(data.view(-1))
                    noise_t = (noise_t / N_BIN - 0.5).view(data.shape)
                else:
                    return lsq
            return torch.where(c1, alpha, torch.where(c2, -alpha, data + noise_t * delta))
        else:
            c1 = data >= alpha
            c2 = data <= 0
            delta = alpha / (2**bit - 1)
            ...
            with torch.no_grad():
                ...
                if hist.sum() != 0:
                    ...
                    return lsq
            return torch.where(c1, alpha, torch.where(c2, 0, data + noise_t * delta))
    else:
        return lsq
...
class Q_Conv2d(nn.Conv2d):
    ...
    def _weight_quant(self):
        ...
        if self.mask is not None:
            weight = self.mask.view_as(self.weight.data) * self.weight
        else:
            weight = self.weight
        return noise_quant(weight, self.bit, self.alpha, self.training, self.noise, True, self.is_stochastic, self.is_discretize)
